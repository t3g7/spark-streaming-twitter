...
object PredictFromStreamRF {
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf()
      .setMaster("local[2]")
      .setAppName("TwitterStreaming Sentiment Prediction with Random Forest classifier")
    ...
    val model = RandomForestModel.load(ssc.sparkContext, trainedModelDir.toString)
